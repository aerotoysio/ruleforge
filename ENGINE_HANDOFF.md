...
- **New node templates** (e.g., a curated `call endpoint` template under the existing `api` category). All declared `NodeCategory` enum values are now implemented in the switch at `RuleRunner.cs:308` — `api` shipped via #3, `sql` was removed via #4 since DocumentForge already covers reads. The `product` node defines the rule's output shape (the rule's "product" — could be an airline product/service, an error envelope, a warning, any structured response). Authors define the shape via `output` literal or `outputSchema` array; placeholders resolve from upstream/ctx/request at evaluation. New domain-flavored templates (e.g. `call endpoint`) are out of scope for THIS pass — see "Schema contract handoff" for the flow.
...
- **All evaluators for declared node categories**: `input | output | iterator | merge | filter | mutator | calc | constant | ruleRef | logic | product | reference | api | bucket | assert | sort | limit | distinct | switch | groupBy`. As of this pass, every declared category has an evaluator in the switch at `RuleRunner.cs:308`. Shipping new evaluators (when adding new categories) is engine-track work. The editor track curates *templates* within those categories.
...
- `ruleforge/src/RuleForge.Core/Graph/RuleRunner.cs:308` — the category switch. All declared categories are implemented inline. `default` still throws `NotSupportedException` to surface enum/switch drift on future additions.
